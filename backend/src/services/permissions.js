...
export function assetFilterForUser(user) {
  ...
  if (isLabTechnician(user) && user.assignedLocationIds?.length) {
    return { locationId: { $in: user.assignedLocationIds } };
  }
  if (canReportOnly(user)) return { assignedTo: user._id };
  return {};
}
...
export function issueFilterForUser(user, assetIds) {
  ...
  return { $or: [{ reportedBy: user._id }, { reporterEmail: user.email?.toLowerCase() }] };
}

/** Get lab technicians for a department based on asset locations */
export async function getLabTechniciansForDepartment(departmentId) {
  const { Asset, User } = await import('../models/index.js');
  const assets = await Asset.find({ departmentId }).select('locationId').lean();
...
export async function assignIssueToUsers(issueId, departmentId) {
  const { Issue, User, Notification } = await import('../models/index.js');
  
  // Get superadmin
  const superadmin = await User.findOne({ role: 'super_admin', isActive: true }).select('_id').lean();
  
  // Get lab technicians for the department
  const labTechnicians = await getLabTechniciansForDepartment(departmentId);
