...
export default function HomeWidgetFilters({
  widget,
  onChange,
  groups,
  templates,
  ...
  const setFilter = (key: keyof HomeWidgetFiltersType, value: string) => {
    onChange({ filters: { ...filters, [key]: value || undefined } });
  };

  const addField = (key: HomeFilterFieldKey) => {
    if (!key || fields.includes(key)) return;
    onChange({ filterFields: [...fields, key] });
  };

  const removeField = (key: HomeFilterFieldKey) => {
    const next = { ...filters };
    delete next[key];
    onChange({ filterFields: fields.filter((f) => f !== key), filters: next });
  };

  const renderField = (key: HomeFilterFieldKey) => {
      ...
      <div className="flex flex-wrap items-end gap-1.5">
        ...
        {available.length > 0 && (
          <div className="shrink-0 min-w-[88px]">
            ...
            <select className={inputClass} value="" onChange={(e) => { const k = e.target.value as HomeFilterFieldKey; if (k) addField(k); }}>
