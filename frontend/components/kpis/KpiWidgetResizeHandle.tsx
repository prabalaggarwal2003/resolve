...
export default function KpiWidgetResizeHandle({
  colSpan,
  rowSpan,
  onResize,
}: {
  ...
  const onPointerDown = useCallback(
    (e: React.PointerEvent<HTMLDivElement>) => {
      ...
      const colWidth = grid.clientWidth / (lg ? KPI_GRID_COLS : 1);
      ...
      const onMove = (ev: PointerEvent) => {
        onResize({
          colSpan: clampKpiSpan(startCol + Math.round((ev.clientX - startX) / colWidth), KPI_MIN_COL_SPAN, KPI_MAX_COL_SPAN),
          rowSpan: clampKpiSpan(startRow + Math.round((ev.clientY - startY) / KPI_ROW_HEIGHT_PX), KPI_MIN_ROW_SPAN, KPI_MAX_ROW_SPAN),
          sizeLocked: true,
        });
      };
      const onUp = () => {
