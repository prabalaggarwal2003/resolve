...
import {
  ...
  PARTNER_MAX_ROW_SPAN,
  PARTNER_MIN_COL_SPAN,
  PARTNER_MIN_ROW_SPAN,
  PARTNER_ROW_HEIGHT_PX,
  clampPartnerSpan,
} from '@/lib/partnerDashboardWidgets';

export default function PartnerWidgetResizeHandle({
  colSpan,
  rowSpan,
  onResize,
}: {
  ...
  const onPointerDown = useCallback(
    (e: React.PointerEvent<HTMLDivElement>) => {
      ...
      const colWidth = grid.clientWidth / (lg ? PARTNER_GRID_COLS : 1);
      ...
      const onMove = (ev: PointerEvent) => {
        onResize({
          colSpan: clampPartnerSpan(
            startCol + Math.round((ev.clientX - startX) / colWidth),
            PARTNER_MIN_COL_SPAN,
            PARTNER_MAX_COL_SPAN
          ),
          rowSpan: clampPartnerSpan(
            startRow + Math.round((ev.clientY - startY) / PARTNER_ROW_HEIGHT_PX),
            PARTNER_MIN_ROW_SPAN,
            PARTNER_MAX_ROW_SPAN
          ),
          sizeLocked: true,
  ...
  return (
    ...
    >
      <svg
        viewBox="0 0 16 16"
        className="w-3 h-3 absolute bottom-1 right-1 text-gray-600 hover:text-violet-400"
        fill="currentColor"
      >
